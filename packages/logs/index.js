module.exports = async function (channel, message, options) {
  const init = new Promise(async resolve => { // Create Promise
    ...
    // Assign Webhook
    let hook = webhooks.find(w => w.name === 'Pogy Logs')
    if (!hook) {
        try {
            hook = await channel.createWebhook('Pogy Logs', {
                avatar: `https://cdn.discordapp.com/attachments/899726940104196137/900081933369868329/logo.png`
            });
        } catch (e) {
            hook = await channel.createWebhook('Pogy Logs', `https://cdn.discordapp.com/attachments/899726940104196137/900081933369868329/logo.png`);
        }
